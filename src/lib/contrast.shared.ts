...
export function capLuminance(hex: string): string {
  ...
  const channel = (i: number) => {
    const scaled = linearToSrgb(
      srgbToLinear(parseInt(hex.slice(i, i + 2), 16) / 255) * k,
    );
    return Math.round(Math.min(1, Math.max(0, scaled)) * 255)
      .toString(16)
      .padStart(2, '0');
  };
...
function bestForeground(backgroundLs: number[]): ContrastCandidate {
  return [WHITE, BLACK, ...ZINC].reduce((best, candidate) =>
    minContrastAgainst(candidate, backgroundLs) >
    minContrastAgainst(best, backgroundLs)
      ? candidate
      : best,
  );
}
...
export function pickForeground(
  ...
  const passing = ordered.filter(
    (candidate) => minContrastAgainst(candidate, backgroundLs) >= minRatio,
  );

  if (passing.length > 0) {
    return strength === 'strong'
      ? passing[0].hex
      : passing[passing.length - 1].hex;
  }
