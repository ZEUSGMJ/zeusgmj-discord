...
function normalizeNameplate(value: unknown): DiscordProfileNameplate | null {
  ...
  const palette =
    ...
    NAMEPLATE_PALETTES.has(raw.palette as DiscordNameplatePalette)
...
export async function fetchDiscordProfile(
  ...
  const rawBadges: Array<Record<string, unknown>> = Array.isArray(json.badges)
    ? json.badges.filter((badge: unknown): badge is Record<string, unknown> =>
        Boolean(badge && typeof badge === 'object'),
      )
    : [];

  const themeColors =
    Array.isArray(userProfile.theme_colors) &&
    userProfile.theme_colors.length >= 2
      ? ([userProfile.theme_colors[0], userProfile.theme_colors[1]] as [
          number,
          number,
        ])
      : null;
