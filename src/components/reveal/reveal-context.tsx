...
export function RevealProvider({ children }: { children: ReactNode }) {
  ...
  const [profileSettle, setProfileSettle] = useState<ProfileSettleEvent | null>(null)
  ...
  const commitIntent = useCallback((next: boolean) => {
  ...
  const emitProfileSettle = useCallback((direction: ProfileSettleDirection, generation: number) => {
  ...
  const onDetailsEnterComplete = useCallback((generation: number) => {
    if (
