...
export type LoadState =
  ...
  | {
      ...
      currentTrack: SpotifyCurrentTrack | SpotifyRecentTrack | null;
...
export function useLanyardPresence(userId: string | undefined): LoadState {
  ...
  const spotifyFallbackPromiseRef =
    useRef<Promise<SpotifyFallbackState> | null>(null);

  const resolveSpotifyFallback = useEffectEvent(
    async (): Promise<SpotifyFallbackState> => {
      const cachedFallback = spotifyFallbackRef.current;
      const now = Date.now();

      if (cachedFallback) {
        const maxAge = cachedFallback.isRecentlyPlayed
          ? RECENT_TRACK_TTL_MS
          : CURRENT_TRACK_TTL_MS;
    ...
    function connect() {
      ...
      ws.onmessage = async (event) => {
        ...
        if (op === 0 && (t === 'INIT_STATE' || t === 'PRESENCE_UPDATE')) {
          const presence = normalizeLanyard(
            d as unknown as Parameters<typeof normalizeLanyard>[0],
          );
